...
- No API-key setup; the hosted beta securely uses TicTic's backend key
...
On first launch, grant Microphone and Accessibility permissions. Each installation receives five total minutes of dictation automatically. Accessibility access is required to observe the global shortcut and insert text at the cursor.
...
The hosted API lives at `https://tictic-api.vercel.app` and uses Vercel Functions plus Upstash Redis. A random installation identifier is generated locally in Keychain and automatically receives a five-minute lifetime allowance—testers never see or enter a code.

## How it works

TicTic records AAC audio locally in 25-second segments and sends each segment through the TicTic Vercel API to Sarvam's `saaras:v3`. The API keeps the shared Sarvam key private and enforces a five-minute lifetime allowance per installation using Redis. The app joins ordered transcripts, applies local vocabulary, optionally polishes the result with `sarvam-105b`, and inserts it into the app that was active when recording began. The random installation identifier lives in Keychain, preferences and transcript history stay on-device, and temporary audio files are removed after each request.

## Privacy

Audio passes through the TicTic API and is forwarded to Sarvam AI for transcription. The API stores only a hashed random installation identifier and the number of milliseconds used; it does not persist audio or transcripts and runs no analytics. Transcript history stays on the Mac and can be disabled or cleared from Settings.
